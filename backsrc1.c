...
static float v[PIXELs+1][HIDDENs+1];
static float w[HIDDENs+1][DIGITs+1];
...
void Init()
{
    ...
    for (j=0; j<=HIDDENs; j++)
        for (i=0; i<=DIGITs; i++)
            w[j][i] = 2.0 * (double) rand() / (double) RAND_MAX - 1;

    for (k=0; k<=PIXELs; k++)
        for (j=0; j<=HIDDENs; j++)
            v[k][j] = 2.0 * (double) rand() / (double) RAND_MAX - 1;
...
{
    FILE *fp;
    static char buffer[H_PIXELs+5];
    unsigned counter, item, i, j, k;
    static float u, rate, threshold, err;
    static float p[DIGITs+1], q[HIDDENs+1], t[DIGITs+1],
           x[PIXELs+1], z[HIDDENs+1], y[DIGITs+1];
    static float dv[PIXELs+1][HIDDENs+1], dw[HIDDENs+1][DIGITs+1];
    counter = 0;
...
int Testing()
{
    ...
    unsigned i, j, k;
    static float t, x[PIXELs+1], z[HIDDENs+1], y[DIGITs+1];
